# Visual Flight Rules (VFR) Within Mactan TMA

1. All VFR aircraft must be able to tune to frequencies in the VATSIM network, including, but not limited to:
    1. 121.800 MHz (RPVM_GND)
    2. 118.100 MHz (RPVM_TWR)
    3. 124.700 MHz (RPVM_APP)
    4. 121.200 MHz (RPVM_S_APP)
    5. 119.300 MHz (MNL_CTR)

2. All arriving VFR flights shall establish contact and remain on listening watch with RPVM_APP or RPVM_S_APP upon entering Mactan TMA or establish two-way radio contact with RPVM_TWR 15 NM to the airport
3. Adhere to established Manila VFR arrival/departure routings
4. Maintain not above 1500 FT within 5 NM to 10 NM from RPVM, not above 2500 FT within 10 NM - 20 NM from RPVM. Entry/exit altitude at NAGA shall not be higher than 1000 FT within 20 NM of RPVM
5. Any deviation from departure / arrival procedures shall have prior approval from RPVM_TWR or higher controller

## VFR Area Chart FIXED WING

![VFR Area Chart RPVM](../../../assets/img/RPVM%20VFR/VFR%20Area%20Chart%20RPVM.png)

## VFR Departures

=== "RWY 04L / RWY 04R"

    North bound aircraft FIXED WING

    1. After airborne, turn left and proceed to LILOAN
    2. Follow shoreline towards east of DANAO or proceed as instructed by ATC

    Southwest bound aircraft FIXED WING

    3. After airborne, turn left towards SM SEASIDE
    4. Proceed NAGA not above 1000 FT, Contact RPVM_APP or proceed as instructed by ATC

    Southeast bound aircraft FIXED WING

    5. After airborne, turn right SOUTHEAST towards PANDANON
    6. Contact RPVM_APP or proceed as instructed by ATC

=== "RWY 22L / RWY 22R"

    North bound aircraft FIXED WING

    1. After airborne, turn right and proceed to LILOAN
    2. Follow shoreline towards east of DANAO or proceed as instructed by ATC

    Southwest bound aircraft FIXED WING

    3. After airborne, turn right towards SM SEASIDE
    4. Proceed NAGA not above 1000 FT, Contact RPVM_APP or proceed as instructed by ATC

    Southeast bound aircraft FIXED WING

    5. After airborne, turn left SOUTHEAST towards PANDANON
    6. Contact RPVM_APP or proceed as instructed by ATC

## VFR Arrivals

=== "RWY 04L / RWY 04R"

    From the North

    1. Report to RPVM_TWR approaching DANAO
    2. Proceed COMPOSTELA
    3. Continue towards CONSOLACION
    4. Obtain clearance to join midfield left downwind 04L/04R or proceed as instructed by ATC

    From the East

    1. Report to RPVM_TWR approaching CAUBYAN
    2. Proceed MABINI
    3. Obtain clearance to join right downwind 04R or 04L, or proceed as instructed by ATC

    From the Southeast

    1. Report to RPVM_TWR approaching COAMEN
    2. Proceed GILUTONGAN
    3. Continue towards PLANTATION BAY
    4. Obtain clearance to join right downwind 04R or 04L or proceed as instructed by ATC

    From the Southwest

    1. Report to RPVM_TWR approaching EAST of NAGA not higher than 1000 FT
    2. Proceed IL CORSO
    3. Obtain clearance to join final O4R or 04L, or proceed as instructed by ATC
        

=== "RWY 22L / RWY 22R"

    From the North

    1. Report to RPVM_TWR approaching DANAO
    2. Proceed COMPOSTELA
    3. Continue towards CONSOLACION
    4. Obtain clearance to join midfield left base 22R or 22L or proceed as instructed by ATC

    From the East

    1. Report to RPVM_TWR approaching CAUBYAN
    2. Proceed MABINI
    3. Obtain clearance to join left downwind 22R or 22L, or proceed as instructed by ATC

    From the Southeast

    1. Report to RPVM_TWR approaching COAMEN
    2. Proceed GILUTONGAN
    3. Continue towards PLANTATION BAY
    4. Obtain clearance to join left downwind 22R or 22L or proceed as instructed by ATC

    From the Southwest

    1. Report to RPVM_TWR approaching EAST of NAGA not higher than 1000 FT
    2. Proceed IL CORSO
    3. Obtain clearance to join midfield downwind 22R or 22L, or proceed as instructed by ATC

## Traffic Patterns

=== "Runway 04L"

    ![RPVM_TrafficPattern_04L](../../../assets/img/RPVM%20VFR/Traffic%20Pattern/TrafficPattern04L.png)

=== "Runway 04R"

    ![RPVM_TrafficPattern_04R](../../../assets/img/RPVM%20VFR/Traffic%20Pattern/TrafficPattern04R.png)

=== "Runway 22L"

    ![RPVM_TrafficPattern_22L](../../../assets/img/RPVM%20VFR/Traffic%20Pattern/TrafficPattern22L.png)

=== "Runway 22R"

    ![RPVM_TrafficPattern_22R](../../../assets/img/RPVM%20VFR/Traffic%20Pattern/TrafficPattern22R.png)

